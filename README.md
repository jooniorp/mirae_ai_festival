...
### 1. 사용자 입력 및 초기화
사용자가 회사명을 입력하면 시스템이 회사명을 추출하고 검증합니다. 메모리 시스템에서 유사한 과거 분석을 검색한 후, 뉴스 트리거 분석을 시작합니다.

### 2. 뉴스 트리거 기반 조건부 실행
뉴스 트리거 분석을 통해 주가 변동 가능성을 판단합니다. 안정적 상태(하/중)로 판단되면 즉시 종료하고, 변동 가능성(상)이 감지되면 추가 분석을 진행합니다.

### 3. 종합 분석 실행 (변동 가능성 감지 시)
데이터를 최신화한 후, ReAct 루프를 통해 동적으로 도구를 실행합니다:

1. **메모리 기반 최적 도구 순서 추천**: 과거 성공 패턴을 학습하여 회사별 최적 조합을 추천
2. **LLM 기반 지능적 도구 선택**: 현재 상황을 분석하여 남은 도구 중 최적을 선택
3. **4개 도구 모두 실행 보장**: 뉴스 트리거, 종토방 여론, 전문가 리서치, 주가 데이터 분석
4. **동적 실행 순서**: 메모리 추천을 반영하되 LLM이 최종 판단하여 최대 8회 반복으로 모든 도구 실행

### 4. 최종 분석 및 결과 출력
모든 도구의 결과를 종합하여 투자 판단(매수/매도/유지)을 생성하고, 근거 분석과 위험/기회 요소를 분석합니다. 구체적인 투자 전략을 제시한 후, 메모리 시스템에 결과를 저장하고 사용자에게 최종 결과를 출력합니다.

### 5. 메모리 시스템 학습
분석 결과의 품질을 평가하고, 도구별 성능을 추적합니다. 성공/실패 패턴을 학습하여 다음 분석을 위한 최적화를 수행합니다.
...
3. **메모리 기반 최적 도구 순서 추천**: 과거 성공 패턴 학습 결과 반영

4. **ReAct 루프 기반 동적 도구 실행**:
   - **LLM 판단**: 현재 상황을 분석하여 다음 실행할 도구 선택
   - **메모리 추천 반영**: 과거 성공 패턴을 참고하되 LLM이 최종 결정
   - **중복 실행 방지**: 이미 실행된 도구는 재실행 불가
   - **4개 도구 모두 실행 보장**: 최대 8회 반복으로 모든 도구 실행

5. **도구 실행 순서 (동적 결정)**:
   - 뉴스 트리거 분석 (첫 번째 고정)
   - 종토방 여론 분석 / 전문가 리서치 분석 / 주가 데이터 분석 (순서는 LLM이 결정)
